({
    ...
    fetchRecords: function(component, event, offset, firepagination){
        try{
             this.callServer(component, "c.fetchRecordsForServerSidePagination", function(response) {
                 var paginationData = response.objectData.lstPaginationData;
                 if (paginationData.length > 0) {
                    component.set("v.totalTableRecords", paginationData);
                }
                 if (firepagination) {
                    this.callPaginationcomptosendnewData(paginationData.length);
                } else {
    ...
    callPaginationcomptosendnewData: function(datalength) {
        try {
            var fireeventtoPagination = $A.get("e.c:Irc_HandleServerSidePagination");
            fireeventtoPagination.setParams({ "newrecordsetlength": datalength });
            fireeventtoPagination.fire();
        } catch (e) {
            console.log(e.stack, true);
        }
    },
